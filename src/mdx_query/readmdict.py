...
from .crypt.ripemd128 import ripemd128
from .crypt.pureSalsa20 import Salsa20

# zlib compression is used for engine version >=2.0
import zlib
# LZO compression is used for engine version < 2.0
try:
    from mdx_query import lzo
except ImportError:
    lzo = None
...
class MDX(MDict):
    ...
    def get_index(self, check_block = True):
        ###  索引列表
        index_dict_list = []
        fd = open(self._fname, 'rb')
        fd.seek(self._record_block_offset)

        num_record_blocks = self._read_number(fd)
        num_entries = self._read_number(fd)
        assert(num_entries == self._num_entries)
        record_block_info_size = self._read_number(fd)
        record_block_size = self._read_number(fd)

        # record block info section
        record_block_info_list = []
        size_counter = 0
        for i in range(num_record_blocks):
            compressed_size = self._read_number(fd)
            decompressed_size = self._read_number(fd)
            record_block_info_list += [(compressed_size, decompressed_size)]
            size_counter += self._number_width * 2
        assert(size_counter == record_block_info_size)
        ...
        for compressed_size, decompressed_size in record_block_info_list:
            current_pos = fd.tell()
            record_block_compressed = fd.read(compressed_size)
            ###### 要得到 record_block_compressed 需要得到 compressed_size (这个可以直接记录）
            ###### 另外还需要记录当前 f 对象的位置
            ###### 使用 f.tell() 命令/ 在建立索引是需要 f.seek()
            # 4 bytes indicates block compression type
